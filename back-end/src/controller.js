...
async function getpayment(request, reply) {
  const cpf = request.params.cpf;

  const payment = await repository.getpayment(cpf);

  if (payment.error) return reply.status(404).json(payment.error);

  reply.json(payment);

}

module.exports = { getAllFoods, getFoodById, setPayment, getpayment};
